...
function moonbitlang$core$builtin$$Map$new$8$(capacity) {
	...
	return {
		...
		list: $make_array_len_and_init(capacity$2, { prev: undefined, next: undefined }),
		size: 0,
		capacity: capacity$2,
...
function moonbitlang$core$builtin$$Hasher$consume4(self, input) {
	self.acc =
		Math.imul(
			moonbitlang$core$builtin$$rotl((self.acc + (Math.imul(input, -1028477379) | 0)) | 0, 17),
			668265263
		) | 0;
}
	...
	while (true) {
		if (!moonbitlang$core$priority_queue$$T$is_empty$0$(queue)) {
									...
									if (i < deps.length) {
										if (moonbitlang$core$array$$Array$op_get$6$(deps, i) === top.node.id) {
											...
											moonbitlang$core$array$$Array$op_set$6$(
												deps,
												i,
												moonbitlang$core$array$$Array$op_get$6$(deps, (deps.length - 1) | 0)
											);
			...
			const cur_index = moonbitlang$core$option$$Option$unwrap$3$(
				moonbitlang$core$array$$Array$search_by$3$(cur_tids, (x) => x === top.tid)
			);
