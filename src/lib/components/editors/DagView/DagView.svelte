...
<script lang="ts">
	...
	function renderRow(row: Row, rowIndex: number, backgroundColor: string) {
		...
		const width =
			(Math.max(row.cur_tids.length, row.output.length, row.input.length) * CELL_SIZE) / 2 + 8;
...
<div class="history-content">
	{#each view.rows as row, index}
		...
		<div
			style="position: relative; height: {CELL_SIZE}px; display: flex; flex-direction: row; align-items: center"
		>
